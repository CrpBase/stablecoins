...
async function getPortfolioBreakdown(address) {
  ...
  // Build the URL for the balances endpoint.  By default we use the
  // Ethereum mainnet path.  You can change "eth-mainnet" to another
  // supported chain (e.g. "base-mainnet", "polygon-mainnet") if your
  // wallet exists on a different network.  We include the API key
  // directly in the query string because many browsers strip custom
  // headers on cross‑origin requests, causing 401 errors.
  const chainName = 'eth-mainnet';
  const baseUrl = `https://api.covalenthq.com/v1/${chainName}/address/${trimmed}/balances_v2/`;
