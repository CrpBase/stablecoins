...
async function getPortfolioBreakdown(address) {
  ...
  // Determine the correct endpoint based on the selected chain. If
  // "allchains" is chosen, use the cross‑chain balances endpoint.
  // Always query the cross‑chain endpoint to aggregate balances from
  // all supported networks for the provided wallet address.
  const baseUrl = `https://api.covalenthq.com/v1/allchains/address/${trimmed}/balances/`;
  // Build the query string.  The API key is included as a query
  // parameter because some browsers strip authorization headers on
  // cross‑origin requests, causing authentication errors.
